<%@ Page Title="" Language="C#" MasterPageFile="~/Bootstrap.Master" AutoEventWireup="true" CodeBehind="TrnStockOutDetail.aspx.cs" Inherits="wfmis.View.TrnStockOutDetail" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type='text/javascript'>
        ...
        var $koNamespace = {};
        var $koStockOutModel;
        var $koStockOutLineModel;
        var $Modal01 = false;
        var $LineUnitId = 0;
        var $LineUnit = "";

        $koNamespace.initStockOut = function (StockOut) {
            var self = this;

            self.PeriodId = ko.observable(!StockOut ? $CurrentPeriodId : StockOut.PeriodId);
            self.Period = ko.observable(!StockOut ? $CurrentPeriod : StockOut.Period);
            self.BranchId = ko.observable(!StockOut ? $CurrentBranchId : StockOut.BranchId);
            self.Branch = ko.observable(!StockOut ? $CurrentBranch : StockOut.Branch);
            self.OTNumber = ko.observable(!StockOut ? "" : StockOut.OTNumber);
            self.OTManualNumber = ko.observable(!StockOut ? "" : StockOut.OTManualNumber);
            self.OTDate = ko.observable(!StockOut ? easyFIS.getCurrentDate() : StockOut.OTDate);
            self.SIId = ko.observable(!StockOut ? 0 : StockOut.SIId);
            self.SINumber = ko.observable(!StockOut ? "" : StockOut.SINumber);
            self.INId = ko.observable(!StockOut ? 0 : StockOut.INId);
            self.INNumber = ko.observable(!StockOut ? "" : StockOut.INNumber);
            self.AccountId = ko.observable(!StockOut ? 0 : StockOut.AccountId);
            self.Account = ko.observable(!StockOut ? "" : StockOut.Account);
            self.ArticleId = ko.observable(!StockOut ? 0 : StockOut.ArticleId);
            self.Article = ko.observable(!StockOut ? "" : StockOut.Article);
            self.Particulars = ko.observable(!StockOut ? "NA" : StockOut.Particulars);
            self.PreparedById = ko.observable(!StockOut ? $CurrentUserId : StockOut.PreparedById);
            self.PreparedBy = ko.observable(!StockOut ? $CurrentUser : StockOut.PreparedBy);
            self.CheckedById = ko.observable(!StockOut ? $CurrentUserId : StockOut.CheckedById);
            self.CheckedBy = ko.observable(!StockOut ? $CurrentUser : StockOut.CheckedBy);
            self.ApprovedById = ko.observable(!StockOut ? $CurrentUserId : StockOut.ApprovedById);
            self.ApprovedBy = ko.observable(!StockOut ? $CurrentUser : StockOut.ApprovedBy);
            self.IsLocked = ko.observable(!StockOut ? false : StockOut.IsLocked);
            self.CreatedById = ko.observable(!StockOut ? $CurrentUserId : StockOut.CreatedById);
            self.CreatedBy = ko.observable(!StockOut ? $CurrentUser : StockOut.CreatedBy);
            self.CreatedDateTime = ko.observable(!StockOut ? "" : StockOut.CreatedDateTime);
            self.UpdatedById = ko.observable(!StockOut ? $CurrentUserId : StockOut.UpdatedById);
            self.UpdatedBy = ko.observable(!StockOut ? $CurrentUser : StockOut.UpdatedBy);
            self.UpdatedDateTime = ko.observable(!StockOut ? "" : StockOut.UpdatedDateTime);

            // Select2 defaults
            $('#SINumber').select2('data', { id: !StockOut ? 0 : StockOut.SIId, text: !StockOut ? "" : StockOut.SINumber });
            $('#INNumber').select2('data', { id: !StockOut ? 0 : StockOut.INId, text: !StockOut ? "" : StockOut.INNumber });
            $('#Account').select2('data', { id: !StockOut ? 0 : StockOut.AccountId, text: !StockOut ? "" : StockOut.Account });
            $('#Article').select2('data', { id: !StockOut ? 0 : StockOut.ArticleId, text: !StockOut ? "" : StockOut.Article });
            $('#PreparedBy').select2('data', { id: !StockOut ? $CurrentUserId : StockOut.PreparedById, text: !StockOut ? $CurrentUser : StockOut.PreparedBy });
            $('#CheckedBy').select2('data', { id: !StockOut ? $CurrentUserId : StockOut.CheckedById, text: !StockOut ? $CurrentUser : StockOut.CheckedBy });
            $('#ApprovedBy').select2('data', { id: !StockOut ? $CurrentUserId : StockOut.ApprovedById, text: !StockOut ? $CurrentUser : StockOut.ApprovedBy });

            if ((!StockOut ? false : StockOut.IsLocked) == true) {
                $(document).find('input[type="text"],textarea').prop("disabled", true);

                $('#Period').prop("disabled", true);
                $('#Branch').prop("disabled", true);
                $('#OTNumber').prop("disabled", true);
                $('#LineCost').prop("disabled", true);
                $('#LineAmount').prop("disabled", true);

                $('#SINumber').select2('disable');
                $('#INNumber').select2('disable');
                ...
                $('#SINumber').select2('enable');
                $('#INNumber').select2('enable');
        ...
        $koNamespace.bindStockOut = function (StockOut) {
            var viewModel = $koNamespace.initStockOut(StockOut);
            ko.applyBindings(viewModel, $("#StockOutDetail")[0]);
            $koStockOutModel = viewModel;
        }
        $koNamespace.getStockOut = function (Id) {
            if (!Id) {
                $koNamespace.bindStockOut(null);
            } else {
                // Render Stock In
                $.ajax({
                    url: '/api/TrnStockOut/' + Id + '/StockOut',
                    cache: false,
                    type: 'GET',
                    contentType: 'application/json; charset=utf-8',
                    data: {},
                    success: function (StockOut) {
                        $koNamespace.bindStockOut(StockOut);

                        // Render Stock In Line
                        $("#StockOutLineTable").dataTable({
                            "bServerSide": true,
                            "sAjaxSource": '/api/TrnStockOut/' + Id + '/StockOutLines',
                            "sAjaxDataProp": "TrnStockOutLineData",
                            "bProcessing": true,
                            "bLengthChange": false,
                        ...
                        $("#JournalEntryTable").dataTable({
                            "bServerSide": true,
                            "sAjaxSource": '/api/TrnJournal/' + Id + '/StockOutJournals',
        ...
        $koNamespace.initStockOutLine = function (StockOutLine) {
            var self = this;

            self.LineId = ko.observable(!StockOutLine ? 0 : StockOutLine.LineId);
            self.LineOTId = ko.observable(!StockOutLine ? 0 : StockOutLine.LineOTId);
            self.LineItemId = ko.observable(!StockOutLine ? 0 : StockOutLine.LineItemId);
            self.LineItem = ko.observable(!StockOutLine ? "" : StockOutLine.LineItem);
            self.LineItemInventoryId = ko.observable(!StockOutLine ? 0 : StockOutLine.LineItemInventoryId);
            self.LineItemInventoryNumber = ko.observable(!StockOutLine ? "" : StockOutLine.LineItemInventoryNumber);
            self.LineParticulars = ko.observable(!StockOutLine ? "NA" : StockOutLine.LineParticulars);
            self.LineUnitId = ko.observable(!StockOutLine ? 0 : StockOutLine.LineUnitId);
            self.LineUnit = ko.observable(!StockOutLine ? "" : StockOutLine.LineUnit);
            self.LineCost = ko.observable(!StockOutLine ? 0 : StockOutLine.LineCost);
            self.LineQuantity = ko.observable(!StockOutLine ? 0 : StockOutLine.LineQuantity);
            self.LineAmount = ko.observable(!StockOutLine ? 0 : StockOutLine.LineAmount);

            return self;
        }
        $koNamespace.bindStockOutLine = function (StockOutLine) {
            try {
                var viewModel = $koNamespace.initStockOutLine(StockOutLine);
                ko.applyBindings(viewModel, $("#StockOutLineDetail")[0]);
                $koStockOutLineModel = viewModel;

                $('#LineItem').select2('data', { id: !StockOutLine ? 0 : StockOutLine.LineItemId, text: !StockOutLine ? "" : StockOutLine.LineItem });
                $('#LineItemInventoryNumber').select2('data', { id: !StockOutLine ? 0 : StockOutLine.LineItemInventoryId, text: !StockOutLine ? "" : StockOutLine.LineItemInventoryNumber });
                $('#LineUnit').select2('data', { id: !StockOutLine ? 0 : StockOutLine.LineUnitId, text: !StockOutLine ? "" : StockOutLine.LineUnit });
            } catch (e) {
                // Fill the controls directly (Multiple binding occurs)
                $('#LineId').val(!StockOutLine ? 0 : StockOutLine.LineId).change();
                $('#LineOTId').val(!StockOutLine ? 0 : StockOutLine.LineOTId).change();
                $('#LineItemId').val(!StockOutLine ? 0 : StockOutLine.LineItemId).change();
                $('#LineItem').select2('data', { id: !StockOutLine ? 0 : StockOutLine.LineItemId, text: !StockOutLine ? "" : StockOutLine.LineItem });
                $('#LineItemInventoryId').val(!StockOutLine ? 0 : StockOutLine.LineItemInventoryId).change();
                $('#LineItemInventoryNumber').select2('data', { id: !StockOutLine ? 0 : StockOutLine.LineItemInventoryId, text: !StockOutLine ? "" : StockOutLine.LineItemInventoryNumber });
                $('#LineParticulars').val(!StockOutLine ? "NA" : StockOutLine.LineParticulars).change();
                $('#LineUnitId').val(!StockOutLine ? 0 : StockOutLine.LineUnitId).change();
                $('#LineUnit').select2('data', { id: !StockOutLine ? 0 : StockOutLine.LineUnitId, text: !StockOutLine ? "" : StockOutLine.LineUnit });
                $('#LineCost').val(!StockOutLine ? 0 : StockOutLine.LineCost).change();
                $('#LineQuantity').val(!StockOutLine ? 0 : StockOutLine.LineQuantity).change();
                $('#LineAmount').val(!StockOutLine ? 0 : StockOutLine.LineAmount).change();
            }
        }
            ...
            if ($Id == "") {
                if (confirm('Save record?')) {
                    $.ajax({
                        ...
                        type: 'POST',
                        contentType: 'application/json; charset=utf-8',
                        data: ko.toJSON($koStockOutModel),
                        success: function (data) {
                            location.href = 'TrnStockOutDetail.aspx?Id=' + data.Id;
                        }
                    }).fail(
                    function (xhr, textStatus, err) {
                        alert(err);
        ...
        function CmdPrint_onclick() {
            if ($Id > 0) {
                window.location.href = '/api/SysReport?Report=StockOut&Id=' + $Id;
            }
        }
        function CmdClose_onclick() {
            location.href = 'TrnStockOutList.aspx';
        }
        function CmdAddLine_onclick() {
            $Id = easyFIS.getParameterByName("Id");
            if ($Id != "") {
                $('#StockOutLineModal').modal('show');
                $Modal01 = true;
                $koNamespace.bindStockOutLine(null);
                // FK
                $('#LineOTId').val($Id).change();
            }
        }
        function CmdEditLine_onclick(LineId) {
            if (LineId > 0) {
                $.ajax({
                    url: '/api/TrnStockOutLine/' + LineId,
                    cache: false,
                    type: 'GET',
                    contentType: 'application/json; charset=utf-8',
                    data: {},
                    success: function (data) {
                        $koNamespace.bindStockOutLine(data);
                        $('#StockOutLineModal').modal('show');
                        $Modal01 = true;
                    }
        ...
        function CmdSaveLine_onclick() {
            ...
            if (LineId != 0) {
                ...
                $.ajax({
                    ...
                    type: 'PUT',
                    contentType: 'application/json; charset=utf-8',
                    data: ko.toJSON($koStockOutLineModel),
                    success: function (data) {
                        location.href = 'TrnStockOutDetail.aspx?Id=' + easyFIS.getParameterByName("Id");
                    }
                })
                .fail(
                function (xhr, textStatus, err) {
        ...
        function CmdApproval_onclick(Approval) {
            ...
            if ($Id != "") {
                if (confirm('Change approval.  Are you sure?')) {
                    $.ajax({
                        url: '/api/TrnStockOut/' + $Id + '/Approval?Approval=' + Approval,
                        cache: false,
                        type: 'PUT',
                        contentType: 'application/json; charset=utf-8',
                        data: {},
                ...
                alert('Stock Out not yet saved.');
            }
        }

        // Select Control
        function Select2_SINumber() {
            $('#SINumber').select2({
                ...
                $('#SIId').val($('#SINumber').select2('data').id).change();
            });
        }
        function Select2_INNumber() {
            $('#INNumber').select2({
                placeholder: 'Stock In',
                allowClear: false,
                ajax: {
                    quietMillis: 150,
                    url: '/api/SelectStockIn',
                    dataType: 'json',
                    cache: false,
                    type: 'GET',
                    data: function (term, page) {
                ...
                $('#INId').val($('#INNumber').select2('data').id).change();
            });
        }
        ...
        $(document).ready(function () {
            ...
            // DatePicker Controls
            $('#OTDate').datepicker().on('changeDate', function (ev) {
                $koStockOutModel['OTDate']($(this).val());
            ...
            $("#StockOutLineTable").on("click", "input[type='button']", function () {
                var ButtonName = $(this).attr("id");
                var Id = $("#StockOutLineTable").dataTable().fnGetData(this.parentNode);
            ...
            // Bind the Page
            if ($Id != "") {
                $koNamespace.getStockOut($Id);
            } else {
                $koNamespace.getStockOut(null);
            }
        });
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    <div id="main-content">
        <div class="container">
            ...
            <section id="StockOutDetail">
                <div class="row">
                    <div class="span6">
                        ...
                        <div class="control-group">
                            <label class="control-label">OUT Number</label>
                            <div class="controls">
                                <input id="OTNumber" type="text" data-bind="value: OTNumber" class="input-medium" disabled="disabled" />
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">OUT Manual Number</label>
                            <div class="controls">
                                <input id="OTManualNumber" type="text" data-bind="value: OTManualNumber" class="input-medium" />
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">OUT Date </label>
                            <div class="controls">
                                <input id="OTDate" type="text" data-bind="value: OTDate" class="input-medium" />
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">Sales Invoice </label>
                            <div class="controls">
                                <input id="SIId" type="hidden" data-bind="value: SIId" class="input-medium" />
                                <input id="SINumber" type="text" data-bind="value: SINumber" class="input-medium" />
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">Stock In </label>
                            <div class="controls">
                                <input id="INId" type="hidden" data-bind="value: INId" class="input-medium" />
                                <input id="INNumber" type="text" data-bind="value: INNumber" class="input-medium" />
                            </div>
                        </div>
                    </div>
                    <div class="span6">
            ...
            <section id="StockOutLine">
                <div class="row">
                    <div class="span12">
                        <table id="StockOutLineTable" class="table table-striped table-condensed">
                            <thead>
                                <tr>
                                    <th colspan="9">
                            ...
                            <tbody id="StockOutLineTableBody"></tbody>
                            <tfoot>
                                <tr>
                                    <td></td>
                                    <td></td>
            ...
            <section id="StockOutLineDetail">
                <div id="StockOutLineModal" class="modal hide fade in" style="display: none;">
                    <div class="modal-header">
                        <a class="close" data-dismiss="modal">×</a>
                        <h3>Stock Out Line Detail</h3>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-horizontal">
                                <div class="control-group">
                                    <div class="controls">
                                        ...
                                        <input id="LineOTId" type="hidden" data-bind="value: LineOTId" class="input-medium" />
                                    </div>
                                </div>
                                <div class="control-group">
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        <input id="LineCost" type="number" data-bind="value: LineCost" class="input-medium pagination-right" data-validation-number-message="Numbers only please" disabled="disabled"/>
                                    </div>
                                </div>
                                <div class="control-group">
